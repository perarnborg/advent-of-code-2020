(() => {
  ...
  const first = run(instructions)

  const runsToEnd = instructionsList => {
    const performedInstructions = []
    let accumulator = 0
    let index = 0
    for (let i = 0; i < instructionsList.length; i++) {
    ...
    return index + 1 === instructionsList.length
  }

  const second = run(instructions.reduce((correctInstructions, instruction, index) => {
    let thisIsTheOne = false
    let changedInstructions = null
    if (instruction.split(' ')[0] === 'nop') {
      changedInstructions = [
        ...instructions.slice(0, index),
        `jpm ${instruction.split(' ')[1]}`,
        ...instructions.slice(index + 1)
      ]
      thisIsTheOne = runsToEnd(changedInstructions)
    }
    if (instruction.split(' ')[0] === 'jmp') {
      changedInstructions = [
        ...instructions.slice(0, index),
        `nop ${instruction.split(' ')[1]}`,
        ...instructions.slice(index + 1)
      ]
      thisIsTheOne = runsToEnd(changedInstructions)
    }
    if (thisIsTheOne) {
      correctInstructions = changedInstructions
    }
